...
const now = hourToMinute('12:40');

const answer = schedule.map(v => {
    const time = hourToMinute(v);
    if (time - now <= 0) {
        return '지나갔습니다';
    } else {
        const minute = parseInt((time - now) % 60)
            .toString()
            .padStart(2, '0');
        const hour = parseInt((time - now) / 60)
            .toString()
            .padStart(2, '0');
